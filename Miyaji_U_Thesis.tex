...
\raisebox{-0.4\height}{\includegraphics[scale=0.4]{images/vanish.JPEG}}
\newpage
...
If we consider graphs, all edges are oriented, and the vertices are given a counterclockwise orientation. Loops are allowed.
...
\begin{definition}
  ...
  For a graph $\Gamma$, a \textit{framed graph} $\mathbf{\Gamma}$ is a pair of 1-dimensional simplicial complex $\Gamma$ and an embedding $\Gamma\hookrightarrow\Sigma$ of $\Gamma$ into a surface $\Sigma$ as a spine. In particular, when $\Gamma$ is a trivalent graph, it is called a \textit{framed trivalent graph}.
  %修正 どっちか消す
  ...
  Moreover, for a framed trivalent graph $\mathbf{\Gamma}=(\Gamma,\Sigma)$, a \textit{knotted trivalent graph (KTG)} $\gamma$ is an embedding of a surface $\Sigma$ into $\RR^3$. The \textit{skeleton} of a KTG $\gamma$ is the trivalent graph $\Gamma$ behind it.
  \begin{figure}[H]
    \centering
...
We identify KTGs whose skeletons, together with their framing, are ambient isotopic. For a trivalent graph $\Gamma$, we denote the vector space over $\QQ$ generated by all linear combinations of KTGs with skeleton $\Gamma$ by
\[
...
\begin{proposition}
  ...
  Two KTGs are isotopic if and only if their graph diagrams are related by a finite sequence of Reidemeister moves $R1', R2, R3$ and $R4$.
