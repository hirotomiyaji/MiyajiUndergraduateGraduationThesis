...
\begin{definition}
  ...
  A \textit{spine} of a simplicial complex $Y$ is a subcomplex $X$ of $Y$ onto which $Y$ collapses, where collapsing means successively removing finite number of pairs of a $k$-simplex $\Delta^k$ and a $(k+1)$-simplex $\Delta^{k+1}$, where $\Delta^{k+1}$ is the unique $(k+1)$-simplex having $\Delta^k$ on its boundary.
\end{definition}
...
\begin{definition}
  Let $\Gamma$ be a trivalent graph and let $\gamma\in \mathcal{K}(\Gamma)$ be a KTG, and $e$ be an edge of $\Gamma$. \textit{Switch the orientation} of $e$ is defined as reversing its orientation, and is denoted by $S_e$. \\
  {\Large 4つの演算の定義未完}
