...
% \includegraphics{images/help.jpg}
\part{Preliminaries}
...
\[\mathcal{F}'_n(\Gamma)\coloneqq\left\{\sum_{i=1}^m a_i \gamma_i\,\middle|\, m\in\NN, a_i\in \QQ, \gamma'_i\colon \Gamma\text{を骨格とし少なくとも}n\text{この特異点を持つKTG}\right\}\]
特異点を解消する写像として，
...
\begin{figure}[htbp]
  ...
  \Huge 図
  \end{figure}
$\mathcal{F}_n\ (n\geq 1)$に対し，$\rho$で送ると，
\[\mathcal{F}_0\supset \rho(\mathcal{F}_1)\supset \rho(\mathcal{F}_2)\cdots.\]
というfiltrationが得られる．このfiltrationに対するassociated graded spaceとして，
\[\mathcal{A}(\Gamma)\coloneqq \prod_{i=0}^{\infty}\mathcal{F}_{i}(\Gamma)/\mathcal{F}_{i+1}(\Gamma)\]
とする．
...
\begin{definition}
  $\Gamma$をKTGとする．$\ck(\Gamma)$ において，係数の和が0となるような形式和全体から生成される集合を$\mathcal{I}(\Gamma)$と書き，$\mathcal{I}\coloneqq \bigcup_{\Gamma}\mathcal{I}(\Gamma)$とする．
\end{definition}
...
\begin{definition}
  $\mathcal{I}^m$を，$\mathcal{I}$の元を少なくとも$m$個含むようなものの演算の合成で生成される$\ck$の部分空間とする．つまり，
  % \[(\mathcal{I})^m\coloneqq\{\gamma\in\mathcal{K}\mid \text{There exist }n, f\colon\prod^n_{i=1}\ck\to\ck, x_1,\dots,x_n\in\ck \text{ such that }\gamma=f(x_1,\dots,x_n), \#\{i\mid x_i\in\mathcal{I}\}\geq m\}.\]
  \[ 
  ...
  \]
\end{definition}
$\mathcal{I}^m$は明らかにfiltrationの構造をもつ．

\begin{lemma}
  $\mathcal{I}(\Gamma)=\{\sum_{i}c_i(\gamma_{1_i} - \gamma_{2_i})\mid \gamma_{1_i}, \gamma_{2_i}\colon \text{generators of }\ck(\Gamma), c_i\in\QQ\}$.
\end{lemma}
...
\begin{theorem}
  $\mathcal{I}^n(\Gamma)=\mathcal{F}_n(\Gamma) \text{ for all } n\geq 0, \Gamma\colon\text{skeleton}$.
\end{theorem}

\begin{proof}
  \begin{enumerate}
    \item $(\mathcal{I}^1=\mathcal{F}_1)$\\
    $(\supset)$ 任意の$\mathcal{F}_1$の元は少なくとも1つのdouble pointを持つような1-singular KTGの交点の正負の差を元に持つため，$\mathcal{F}_1\subset \mathcal{I}\subset\mathcal{I}^1$. \\
    $(\subset)$ 任意の$\mathcal{I}$の元は，$\sum_i c_i(\gamma_i-\gamma'_i)$と書ける．$\mathcal{F}_1$において，同じskeleton を持つ任意の2つのKTGはcrossing change により移りあうため，$\gamma_i - \gamma'_i$を1点における正負の交差の差 $\tilde{\gamma}_i-\tilde{\gamma}'_i$となるようにできる．よって
    \[\sum_{i}c_i(\gamma_i-\gamma'_i)=\sum_{i}c_i(\tilde{\gamma}_i-\tilde{\gamma}'_i)\in\mathcal{F}_1.\]
  \end{enumerate}
\end{proof}
