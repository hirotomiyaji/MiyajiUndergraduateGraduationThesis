%! TeX root = MiyajiUndergraduateGraduationThesis.tex
\documentclass[
  a4paper,
  12pt,
  dvipdfmx
...
全ての辺は向きづけられているものとし，頂点は回転するように向きを与える．ループや円などの辺を許す．

\begin{definition}
  \textit{Surface}とは第2可算公理を満たす2次元多様体をいう．
\end{definition}
\begin{definition}
  単体複体$Y$における\textit{spine}とは，$Y$の部分単体複体$X$出会って，
\end{definition}

\begin{definition}
  Trivalent graph $\Gamma$に対し，\textit{its thicking} or \textit{framed graph}[Th]$\mathbb{\Gamma}$とは，頂点を太らせたもの．[Th]を参照すると，1次元単体複体$\Gamma$と，surface $\Sigma$に対し，そのspineとなるように$\Gamma$を埋め込んだもの(なめらか？)の組．
\end{definition}
